...
from predictor.model import writeTheta, readTheta, printAndFlush, trainModelParallel, testModelParallel, \
    recordPredictions, loadOligoFeaturesAndReadCounts
...
def runAnalysis(guideset_file):
    ...
    for i, (train_idx, test_idx) in enumerate(kf.split(guideset)):
        ...
        if os.path.isfile(tmp_file):
            ...
            test_set = [x for x in ([y for y in train_set] + [y for y in test_set]) if x not in rec_train_set][
                       :int(NUM_OLIGO / 2)]
...
if __name__ == '__main__':
    ...
    runAnalysis('FORECasT/guideset_data.txt')
