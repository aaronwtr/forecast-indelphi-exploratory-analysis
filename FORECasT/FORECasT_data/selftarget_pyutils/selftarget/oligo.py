...
GUIDESET_HEADER = 'ID	Guide Sequence	TargetSequence	Scaffold	Subset	Comments	PAM Index	Strand'
...
def splitOligoFastaFile(filename, oligo_ids, filepath=None):
    if filepath is not None:
        out_filename = filepath + '/' + filename.split('/')[-1][:-6]
    else:
        out_filename = filename[:-6]
...
def getOligoIdxFromId(oligo_id):
    if oligo_id != GUIDESET_HEADER:
        return eval(oligo_id[5:])
...
def partitionGuides(lib='Both', oligo_detail_dir='../ST_June_2017/data'):
    ...
    partitions = {'All': set(), 'Non-Targeting no MH>3': set(), 'Real Guides': set(), 'PAM': set(), 'Mismatch': set(),
                  'Non-Targeting with MH>3': set(), 'Non-Targeting': set(), 'With MH>3': set(), 'No MH>3': set()}
